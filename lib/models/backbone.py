...
def get_model_path(modelname):
    ...
    PTDICT = {
        ...
        'swin_B_224_22k': 'swin_base_patch4_window7_224_22k.pth',
        'swin_S_224_22k': 'swin_small_patch4_window7_224_22k.pth',
...
class BackboneBase(nn.Module):
    ...
    def forward(self, input: Tensor):  # input: [bs, 3, 448, 448]
        # pdb.set_trace()
        # print("Backbone Base forward")
        xs = self.body(input)  # xs: [bs, 2048, 14, 14]
        out: Dict[str, Tensor] = {}
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        # args.backbone, train_backbone, return_interm_layers, False, args.pretrained
...
class Joiner(nn.Sequential):
    ...
    def forward(self, input: Tensor): # input: [bs, 3, 448, 448]
        xs = self[0](input)  # xs['0']: [bs, 2048, 14, 14] resnet-101
        out: List[Tensor] = []
        pos = []
        if isinstance(xs, dict):
            ...
            for name, x in xs.items():
                out.append(x)
                # position encoding
                pos.append(self[1](x).to(x.dtype))  # self[1](x): [bs, 2048, 14, 14]
        else:
...
def build_backbone(args):
    position_embedding = build_position_encoding(args)  # [bs, num_pos_feats, maxH, maxW]
    train_backbone = True
    if args.backbone in ['swin_S_224_22k', 'swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k']:
    ...
    model = Joiner(backbone, position_embedding, args)  # 继承自sequential
    # model = Joiner(backbone, args)  # 继承自sequential
